...
constructor() {
    ...
    fun mapDtoToDbModel(dto: LocationsDto) =
        LocationDbModel(
            id = dto.id,
            name = dto.name,
            type = dto.type,
    ...
    fun mapDbModelsListToEntitiesList(dbModels: List<LocationDbModel>) =
        dbModels.map(
            ::mapDbModelToEnity
        )

    fun mapDbModelToEnity(dbModel: LocationDbModel) =
        LocationEntity(
            id = dbModel.id,
            name = dbModel.name,
            type = dbModel.type,
            ...
            residentsId =
            if (dbModel.residentsId.isNotEmpty()) {
